# Enterprise-Windows-Server-2019-ad-lab
OS: Windows Server 2019
Client OS: Windows 10
Role: Active Directory Domain Services
Role: DNS & DHCP
Platform: Oracle VM VirtualBox
Verification: Verified Lab
...
Created an isolated virtual network in Oracle VM VirtualBox with subnet 192.168.10.0/24.
Crucial Enterprise Step: Disabled VirtualBox's internal DHCP engine to eliminate rogue DHCP conflicts and guarantee that CORP-DC01 functions as the sole authoritative DHCP server.
Phase 2: Windows Server 2019 Baselining (GUI)
Provisioned CORP-DC01 with 2 GB RAM, 1 vCPU, and a 20 GB dynamically allocated virtual hard disk.
Installed Windows Server 2019 Standard (Desktop Experience).
Assigned a static IPv4 address via Network Connections (ncpa.cpl):
IP Address: 192.168.10.10
Subnet Mask: 255.255.255.0
Default Gateway: 192.168.10.1
Preferred DNS Server: 127.0.0.1 (Points to local DNS service).
Renamed the computer to CORP-DC01 in System Properties (sysdm.cpl) and rebooted.
Phase 3: AD DS Role Installation & Forest Promotion (Server Manager GUI)
Opened Server Manager > Manage > Add Roles and Features.
Selected Role-based or feature-based installation targeting CORP-DC01.
Checked Active Directory Domain Services (automatically installing ADUC, ADAC, and management snap-ins) and DNS Server. Completed the wizard.
Clicked the post-deployment flag notification in Server Manager and selected "Promote this server to a domain controller":
...
Passed all prerequisite checks and clicked Install. The server rebooted automatically upon promotion.
Phase 4: DNS Server Configuration (DNS Manager GUI)
Opened DNS Manager (dnsmgmt.msc).
Confirmed the authoritative Forward Lookup Zone corp.local with operational SRV records (_ldap, _kerberos).
...
In Server Manager, installed the DHCP Server role.
Completed the post-install task to Authorize the DHCP server in Active Directory using domain admin credentials (corp.local\Administrator).
Opened DHCP Management Console (dhcpmgmt.msc):
...
Provisioned workstation CORP-CLO1 (Windows 10 Enterprise) attached to LabNetwork.
Verified automated IP assignment (192.168.10.50) and DNS (192.168.10.10) via DHCP.
Joined CORP-CLO1 to corp.local via System Properties (sysdm.cpl), authenticated with domain administrative credentials, and restarted upon receiving "Welcome to the corp.local domain".
Created domain user account Chukwubuzor E. Perazim in Active Directory (CN=Users,DC=corp,DC=local) and validated interactive domain logon.
...
Domain Controller nltest and whoami Output
...
Client Workstation Verification
...
DNS Architecture & DC Loopback Binding:
...
Active Directory Authorized DHCP Protection:
...
Subnet Separation & Infrastructure Reservation:
...
LinkedIn: linkedin.com/in/ChukwubuzorPerazim
Email: chukwubuzorperazim@gmail.com
GitHub: github.com/ChukwubuzorPerazim
